...
def sort_steps(lesson, results):
    steps = []
    sorted_steps = []
    for key in results.keys():  # Name of step here https://stepik.org/api/steps/3145280, but not useful
...
def main():
    ...
    for user in course_grades['course-grades']:
        user_meta = requests.get(args.url + '/users/' + str(user['user']),
                                   headers={'Authorization': 'Bearer ' + token})
        ...
        grades = ({'full name': full_name, 'last viewed': last_viewed, 'total score': user['score']})
